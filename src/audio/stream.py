...
def list_audio_devices() -> AudioDeviceMap:
    ...
    if IS_WINDOWS:
        _add_windows_proc_exclude_device(result, seen_names)
...
def _add_windows_proc_exclude_device(result: AudioDeviceMap, seen: set[str]) -> None:
    ...
    try:
        ...
        return
    if not is_process_exclude_available():
        return
    key = PROC_EXCLUDE_DEVICE_ID.lower()
    if key in seen:
        return
    result["input"].insert(
        0,
        {
            "id": PROC_EXCLUDE_DEVICE_ID,
            "name": "[推荐] 系统声音（免驱动·排除本应用）",
            "input_channels": 2,
            "output_channels": 0,
            "sample_rate": 48000,
        },
...
def find_device_by_name(name_pattern: str, kind: str = "output") -> int | str | None:
    """
    根据名称模糊匹配音频设备。
    ...
    for idx, dev in enumerate(devices):
        ...
        if is_valid and name_pattern.lower() in dev_name:
            return idx
...
class AudioStream:
    ...
    def _open_windows_proc_exclude(self) -> None:
        """Driverless capture that excludes this process from the mix."""
        from src.audio.wasapi_process_loopback import ProcessExcludeLoopback
